...
class EmbeddingPacker:
    ...
    def __init__(self, g: Network, sink: AgentId, sink_embedding: torch.Tensor, other_nodes: List[AgentId]):
        """
    ...
    def compute_objective(self, embedding_dict: OrderedDict[AgentId, torch.Tensor],
        ...
        for diverter in nontrivial_diverters:
            ...
            print('HELLLOOOOO')
            current_neighbors = torch.DoubleTensor(self._g.get_nebr_embs(diverter))
            neighbor_embeddings = torch.cat([embedding_dict[current_neighbor]
                                             for current_neighbor in current_neighbors])
            q_values = self._g.calc_q_vals(diverter_embeddings, sink_embeddings, neighbor_embeddings).flatten()
